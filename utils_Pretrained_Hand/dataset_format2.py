...
def make_basis(x_dir, y_dir):
    #手の平の平面ベクトルを計算し、その法線ベクトルを求めると手の甲ベクトルになる。
    x = x_dir / np.linalg.norm(x_dir)
    y = y_dir - np.dot(x, y_dir) * x  # x に直交な成分を取り出す
...
def format_hand(hand):
    ...
    vec_ring = hand[21]
    """
    vec_middle = hand[7] # 人差し指第3関節
    vec_ring = hand[13] # 薬指第3関節

    vec_tumb = vec_tumb / np.linalg.norm(vec_tumb)
    palm = np.cross(vec_middle, vec_ring)

    R_c = make_basis(vec_tumb, palm) #手の甲ベクトル
    R_target = np.eye(3)
    R_align = R_target @ R_c.T
    hand_format = hand @ R_align.T
    return hand_format, R_align.T, vec_tumb
