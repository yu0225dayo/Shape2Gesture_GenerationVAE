...
if __name__=="__main__":
    ...
    state_dict_pointnet = torch.load("save_pretrained_partsseg/pointnet_acc_partseg_best.pth", weights_only=True)
    pointnet.load_state_dict(state_dict_pointnet)
    pointnet.eval()

    #学習済みvae
    parts_encoder_l, parts_encoder_r = PartsEncoder_w_TNet(), PartsEncoder_w_TNet()
    state_parts_e_l = torch.load("save_model/fps_formatxy2/parts_encoder_l_best.pth", weights_only=True)
    state_parts_e_r = torch.load("save_model/fps_formatxy2/parts_encoder_r_model_best.pth", weights_only=True)
