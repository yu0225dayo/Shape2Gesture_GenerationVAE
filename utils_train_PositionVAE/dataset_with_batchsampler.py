...
# =============================================================
# BatchSampler: 1 batchのうち指定割合を bottle("bo") に固定
# 例) batch_size=16, bottle_ratio=0.25 -> bottle=4, other=12
# =============================================================
import math
import random
from torch.utils.data import Sampler

class BottleQuotaBatchSampler(Sampler):
    """BatchSampler that enforces a fixed quota of bottle samples per mini-batch.

    This sampler expects dataset.datapath entries to be:
        [label_id, pts_path, label_path, hand_path, csvname]
    where label_id is like "bo", "mu", ...

    Notes:
      - bottle indices are sampled cyclically (with replacement if needed)
      - other indices are consumed without replacement per epoch as much as possible
      - epoch length is defined by how many full 'other' groups we can make
    """
    ...
    def __init__(
        ...
        self.bottle_ratio = float(bottle_ratio)
        self.drop_last = bool(drop_last)
        self.seed = int(seed)
        self.shuffle_within_batch = bool(shuffle_within_batch)

        # compute quota
        self.n_bottle = max(1, int(round(self.batch_size * self.bottle_ratio)))
        self.n_other = self.batch_size - self.n_bottle
        if self.n_other <= 0:
            raise ValueError("bottle_ratio が大きすぎます（other が 0 以下になります）")

        # build index pools
        self.bottle_idx = []
        self.other_idx = []
        for i, item in enumerate(getattr(dataset, "datapath", [])):
            label_id = item[0]  # "bo", "mu", ...
            if label_id == self.bottle_label:
                self.bottle_idx.append(i)
            else:
                self.other_idx.append(i)

        if len(self.bottle_idx) == 0:
            raise ValueError(f"bottle_label='{self.bottle_label}' のサンプルが0です（dataset.datapath の先頭要素を確認）")
        if len(self.other_idx) == 0:
            raise ValueError("bottle 以外のサンプルが0です（select_labels や dataset を確認）")

        # number of batches per epoch (defined by others)
        if self.drop_last:
            self.num_batches = len(self.other_idx) // self.n_other
        else:
            self.num_batches = math.ceil(len(self.other_idx) / self.n_other)

    def __len__(self):
        return self.num_batches

    def set_epoch(self, epoch: int):
        # call this each epoch to change shuffling
        self.seed = int(self.seed) + int(epoch)

    def __iter__(self):
        rng = random.Random(self.seed)

        bottle = self.bottle_idx.copy()
        other = self.other_idx.copy()
        rng.shuffle(bottle)
        rng.shuffle(other)

        bptr = 0  # cyclic pointer for bottle

        for bi in range(self.num_batches):
            start = bi * self.n_other
            end = start + self.n_other
            other_batch = other[start:end]

            if len(other_batch) < self.n_other:
                if self.drop_last:
                    break
                # pad by cycling from the beginning
                need = self.n_other - len(other_batch)
                other_batch = other_batch + other[:need]

            bottle_batch = []
            for _ in range(self.n_bottle):
                bottle_batch.append(bottle[bptr % len(bottle)])
                bptr += 1

            batch = bottle_batch + other_batch
